...
import gevent, termcolor, time, argparse, sys, os
...
if cuauv_vehicle:
    sys.stdout.write(termcolor.colored('Running {0} tests from environment variable CUAUV_VEHICLE\n'.format(cuauv_vehicle), 'yellow'))
...
for t in gen.tests:
    if any(t.name.startswith(p) for p in prefixes_to_run) or \
            not any(vehicle in t.name for vehicle in CUAUV_VEHICLES):
        greenlets.append(gevent.spawn(run, t))

gevent.joinall(greenlets)

sys.exit(sum(x.get() for x in greenlets))
