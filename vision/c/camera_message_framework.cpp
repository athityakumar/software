...
bool read_frame(struct frame* frame, message_framework_p framework_w) {
  if (!framework_w->live) {
    return false;
  }
  ...
  pthread_mutex_unlock(&framework->cond_mutex);
...
message_framework_p create_message_framework(std::string direction,
      ...
      if (!owned && legal_size) {
        ...
        if (owned)
          std::cout << direction << " is currently owned by some other process. "
                    << "Make sure that no other capture source is trying to access this file, or run \"auv-visiond cleanup\""
                    << std::endl;
        else if (!legal_size)
  ...
  return initialize_message_framework(framework);
}
