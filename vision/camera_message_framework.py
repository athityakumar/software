from ctypes import POINTER, c_ubyte, c_uint32, c_uint64, c_char_p, c_voidp, c_ssize_t, c_bool, addressof, Structure, create_string_buffer
import signal
import time
import sys
import struct
...
_lib_nothread.read_frame.restype = c_bool
...
class Accessor:
    ...
    def get_next_frame(self):
        ...
        frame = self._frame
        is_live = _lib.read_frame(addressof(frame), self._framework)
        if not is_live:
            self.cleanup()
            raise StateError('Accessor has already been cleaned up!')
        shape = frame.height, frame.width, frame.depth
        res = np.ctypeslib.as_array(frame.data, shape), frame.acq_time
...
class ModuleFrameworkAccessor(Accessor):
    ...
    def _watch_image(self, image_name, image):
        while running and image_name in self._image_accessors:
            next_value, acq_time = image.get_next_frame()
...
class ModuleFrameworkCreator(ModuleFrameworkAccessor, Creator):
    def __init__(self, module_name):
        ...
        self._option_ordering = lambda x: x
