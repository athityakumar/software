...
from capture_sources import CaptureSource
...
# run a module forever. this is intended to be ran as a separate process
    ...
    def update_option_watcher(option_name, option_value):
        ...
        with option_lock:
            try:
                ...
                old_value = module.options_dict[option_name].value
...
def daemon_running():
    filenames = [filename for filename in os.listdir('{}/pids'.format(dirname))]
    running = False
    for filename in filenames:
        ...
        try:
            ...
            print("Removing .pid for {}".format(full_path))
            os.remove("{}/pids/{}".format(dirname, filename))
    return running
...
def stop_daemon(should_wait=True):
    ...
    for filename in (filename for filename in os.listdir('{}/pids'.format(dirname)) if '.pid' in filename):
