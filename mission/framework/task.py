...
class Task:
    ...
    def log(*args, level, copy_to_stdout = True, **kwargs) -> None:
        ...
        """
        Logger(Task.task_call_stack + [level])(*args, copy_to_stdout=copy_to_stdout, **kwargs)

    logv = functools.partialmethod(log, level="verbose", copy_to_stdout=False)
    logd = functools.partialmethod(log, level="debug", copy_to_stdout=False)
    logi = functools.partialmethod(log, level="info", copy_to_stdout=False)
    logw = functools.partialmethod(log, level="warn", copy_to_stdout=True)
    loge = functools.partialmethod(log, level="error", copy_to_stdout=True)
