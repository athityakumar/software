...
SceneObject skybox_scene_object;
...
int load_axes(SceneObject *scene_object) {
  auto renderable = get_renderable(&color_direct);
  renderable_clumps.push_back(std::make_unique<std::vector<Renderable>>());
  renderable_clumps[renderable_clumps.size() - 1]->push_back(std::move(renderable));
  color_direct.add_object(scene_object, renderable_clumps[renderable_clumps.size() - 1].get());
  return 0;
}
...
glm::mat4 compute_mvp_matrix(glm::vec3 pos, glm::vec3 direction, glm::vec3 up,
  ...
  return glm::infinitePerspective(fov, aspect_ratio, 0.1f) *
