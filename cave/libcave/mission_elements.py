...
class OrangeBuoy(MissionElement):
    name = "Buoy (Orange)"
    def __init__(self):
        MissionElement.__init__(self, shm.buoy_settings.enabled, shm.orange_results)
    def init(self): #overloaded
        MissionElement.init(self)
        shm.buoy_settings.color.set("orange")
    def get_x_y(self):
        return shm.orange_results.center_x.get(), shm.orange_results.center_y.get()
    def get_visible(self):
        return shm.orange_results.probability.get() > 0.5
...
class Pipe(MissionElement):
    ...
    def __init__(self):
        ...
        MissionElement.__init__(self, shm.pipe_settings.enabled, shm.pipe_results3)
        self.reset()
    def reset(self):
        self.results_groups = [shm.pipe_results, shm.pipe_results2, shm.pipe_results3]
    def get_x_y(self):
        return self.results_groups[-1].center_x.get(), self.results_groups[-1].center_y.get()
    def get_visible(self):
        return self.results_groups[-1].probability.get() > 0.5
    def get_heading_delta(self):
        return self.results_groups[-1].heading_delta.get()
    def get_next(self): #override
        self.results_groups.pop()
        return len(self.results_groups) > 0 #have next if there are items left to process
