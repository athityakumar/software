...
class GenericThruster(object):
    ...
    def __init__(self, yaw, pitch, position, drag=1.0, link=None, name="", reversed_polarity=False, broken=False):
        """
        ...
        q = quat.Quaternion(hpr=(yaw % 360, pitch, 0.0))
        self.force_hat = q * np.array((1, 0, 0))
        self.torque_hat = np.cross(np.array(position), self.force_hat)
        ...
        self.min_thrust = self.pwm_to_thrust(self.min_pwm)
        self.min_neg_thrust = self.pwm_to_thrust(-self.min_pwm)

        # Variable-s used for quadratic equation in thrust_to_pwm
    ...
    def thrust_to_pwm(self, thrust):
        ...
        # pick the one of -min_pwm, 0, min_pwm which is the closest
...
class T100(GenericThruster):
    ...
    curve_reverse = BLUEROBOTICS_T100

class T200(GenericThruster):
    max_pwm = 255
    min_pwm = 1
    curve_forward = BLUEROBOTICS_T200
    curve_reverse = BLUEROBOTICS_T200
