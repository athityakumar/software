...
cflags = ['-pthread', '-lrt', '-lm', '-Wall', '-I.', '-ggdb2', '-O3', '-Werror',
          '-Wno-ignored-attributes', '-Wno-misleading-indentation']
cppflags = ['-std=c++14', '-fdiagnostics-color']
...
n.rule('stack',
  command = 'stack --verbosity 0 --stack-yaml $config install --local-bin-path $bin',
  description = 'Haskell Stack { `stack --verbosity 0 --stack-yaml $config install --local-bin-path $bin` }',
  restat = True)
